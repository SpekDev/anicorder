...
from textual.widgets import Label, Button, Input, Footer, DataTable, Select, MaskedInput
...
class Manage_Anime_Screen(Screen):
	...
	def compose(self) -> ComposeResult:
		...
		yield DataTable(id="manage_data_table",cursor_type="row", zebra_stripes=True, show_cursor=True)
...
class Record_Anime_Screen(Screen):
	...
	def compose(self) -> ComposeResult:
		...
		yield Button("Record Anime", id="record_anime_button", disabled=True, classes="invalid")

	def on_input_submitted(self):
		normal_input = titlecase(self.screen.query_one("#record_anime_input", Input).value).strip()
		if  self.app.record_check_anime(normal_input) is None:
			...
			self.notify("Entry already exists ",title="Database Message", severity="error")
			self.app.action_quit()
...
class Connect_DB_Screen(Screen):
	...
	def compose(self) -> ComposeResult:
		...
		yield Input(value="localhost", valid_empty=False, id="db_server",  placeholder="IP Address")
		yield Input(value="anicorder",valid_empty=False, id="db_db", placeholder="Database Name")
		yield Input(value="anime",valid_empty=False, id="db_table",  placeholder="Database Table")
		yield Input(value="root", valid_empty=False, id="db_username", placeholder="Database Username") # Ensure user has read and append permissions
		yield Input(id="db_pass", password=True ,select_on_focus=True, placeholder="Database Password")
...
class Interactive_Manage_Flyout(ModalScreen):
	...
	BINDINGS = [
		...
		Binding("h, left, a", "app.go_left", "Move left or back", show=False), # Vim style binds
	...
	def __init__(self, anime):
		super().__init__()
		self.anime_id, self.english_title, self.status, self.episode, self.time_in_seconds = anime
		...
		with Container():
			...
			with Horizontal(id="episode_timestamp_container"):
				with Horizontal():
					...
					yield MaskedInput(template="9999", id="episode_entry", value=str(self.episode))
				with Horizontal():
					yield Label("Timestamp: ")
					yield MaskedInput(template="99:99:99", id="time_column_entry", value=convert_seconds_to_column(self.time_in_seconds))
			with Horizontal(id="status_container"):
				...
				id="status_select", type_to_search=False, value=self.status, compact=True, allow_blank=False)

			yield Button("Remove Entry", id="remove_entry_details",action=f"app.remove_entry({self.anime_id})")
			yield Button("Modify Entry", id="modify_entry_details",action=f"app.modify_entry({self.anime_id})", disabled=True)
...
class Anicorder(App):
	...
	BINDINGS = [
		Binding("q, ctrl+q, escape", "quit", "Quit"), # Pops the screen, override to quit the app if it is the last screen on stack
		Binding("r", "toggle_theme", "Toggle Theme"),
		Binding("h, left, a", "go_left", "Move left or back", show=False), # Vim style binds
	...
	def compose(self) -> ComposeResult:
		...
		yield Button("Manage Anime", id='home_manage', variant='default', classes="home", disabled=True)
		yield Button("Record Anime", id='home_record', variant='default', classes="home", disabled=True)
	...
	def action_connect_db(self):
		...
		if self.conn is None:
			try:
				...
				self.conn = mariadb.connect(host=server, database=db, user=user, password=password, connect_timeout=7, ssl=True)
				self.notify(f"Connection to Database established", title="Connection Status", severity="information", timeout=5)
				...
				self.conn = None
				self.notify(f"Connection to Database Failed!\nError: {e}", title="Connection Status",severity="error", timeout=7)
	...
	def action_query(self) -> None:
		...
		if (input_text := self.screen.query_one("#manage_anime_input", Input).value.strip()) == "":
	...
	def record_check_anime(self, title):
		with self.conn.cursor() as cur:
			cur.execute(r"SELECT english_title FROM anime WHERE english_title=?", (title,))
			return cur.fetchone()

	def action_confirmation_screen(self):
		pass
	...
	def action_modify_entry(self,anime_id):
		...
		try:
			...
			self.notify("Please enter a epidsode value",title="Data Error",severity="error")
		try:
			new_time = convert_column_to_seconds(self.screen.query_one("#time_column_entry", MaskedInput).value)
		except ValueError:
			self.notify("Please enter a time value",title="Data Error",severity="error")
			return
		...
		with self.conn.cursor() as cur:
			...
			self.conn.commit()
		self.app.action_quit()
		self.notify(f"Entry Updated!", title="Database Message" ,severity="information", timeout=3)
	...
	def action_remove_entry(self, anime_id):
		with self.conn.cursor() as cur:
			...
			self.conn.commit()
		self.app.action_quit()
		self.notify(f"Entry Removed!", title="Database Message" ,severity="warning", timeout=3)
